...
class TestRAGSystem:
    ...
    def test_query_successful_with_tool_use(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            # Setup mocks
            mock_ai_gen.return_value.generate_response.return_value = "Based on the course content, here's the answer."
            mock_session.return_value.get_conversation_history.return_value = None
            
            rag_system = RAGSystem(test_config)
            
            # Mock tool manager to return sources
            rag_system.tool_manager.get_last_sources = Mock(return_value=["Course 1 - Lesson 1"])
    ...
    def test_query_with_session_history(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            # Setup mocks
            mock_ai_gen.return_value.generate_response.return_value = "Follow-up response."
            mock_session.return_value.get_conversation_history.return_value = "Previous conversation"
            ...
            response, sources, source_links = rag_system.query("Follow up question", session_id="session123")
            
            # Assert
            assert response == "Follow-up response."
            
            # Verify session history was used
            mock_session.return_value.get_conversation_history.assert_called_once_with("session123")
            call_args = mock_ai_gen.return_value.generate_response.call_args[1]
            ...
            mock_session.return_value.add_exchange.assert_called_once_with(
                "session123", "Answer this question about course materials: Follow up question", "Follow-up response."
            )
    ...
    def test_query_failed_scenario_max_results_zero(self, broken_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_ai_gen.return_value.generate_response.return_value = "I couldn't find any relevant information."
    ...
    def test_query_with_no_session(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_ai_gen.return_value.generate_response.return_value = "Response without session."
    ...
    def test_add_course_document_success(self, test_config, sample_course):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            # Setup mocks
            mock_chunks = [
                CourseChunk(content="chunk1", course_title="Test Course", chunk_index=0),
                CourseChunk(content="chunk2", course_title="Test Course", chunk_index=1)
            ]
            mock_doc_proc.return_value.process_course_document.return_value = (sample_course, mock_chunks)
            ...
            mock_doc_proc.return_value.process_course_document.assert_called_once_with("/path/to/course.pdf")
            ...
            mock_vector_store.return_value.add_course_content.assert_called_once_with(mock_chunks)
    ...
    def test_add_course_document_error(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            # Execute
            course, chunk_count = rag_system.add_course_document("/path/to/bad_course.pdf")
    ...
    def test_add_course_folder_success(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            mock_listdir.return_value = ["course1.pdf", "course2.txt", "course3.docx", "ignore.jpg"]
            ...
            # Execute
            total_courses, total_chunks = rag_system.add_course_folder("/path/to/docs", clear_existing=False)
    ...
    def test_add_course_folder_skip_existing(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_proc, \
            ...
            mock_chunks = [CourseChunk(content="content", course_title="Existing Course", chunk_index=0)]
            mock_doc_proc.return_value.process_course_document.return_value = (existing_course, mock_chunks)
    ...
    def test_end_to_end_query_flow_integration(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            mock_session.return_value.get_conversation_history.return_value = None
            mock_ai_gen.return_value.generate_response.return_value = "Comprehensive answer based on course materials."
            
            rag_system = RAGSystem(test_config)
            rag_system.tool_manager.get_last_sources = Mock(return_value=["Complete Course - Lesson 5"])
            rag_system.tool_manager.get_last_source_links = Mock(return_value=["https://example.com/lesson5"])
            
            # Execute complete flow
            response, sources, source_links = rag_system.query("Explain the complete concept from lesson 5")
            ...
            call_args = mock_ai_gen.return_value.generate_response.call_args[1]
            assert call_args["query"] == "Answer this question about course materials: Explain the complete concept from lesson 5"
            assert "tools" in call_args
            assert "tool_manager" in call_args