...
class TestAIGenerator:
    ...
    def test_api_parameters_consistency(self, mock_anthropic_client):
        ...
        with patch('ai_generator.anthropic.Anthropic') as mock_anthropic:
            ...
            generator.generate_response("Second question", conversation_history="Previous context")
    ...
    def test_sequential_tool_calling_two_rounds(self, mock_tool_manager):
        ...
        with patch('ai_generator.anthropic.Anthropic') as mock_anthropic:
            ...
            # Assert
            assert response == "Based on the course outline and lesson content, here's the answer."
    ...
    def test_sequential_tool_calling_early_termination(self, mock_tool_manager):
        ...
        with patch('ai_generator.anthropic.Anthropic') as mock_anthropic:
            ...
            round2_response.content = [Mock()]
            round2_response.content[0].text = "Here's the direct answer based on the search results."
            ...
            mock_tool_manager.execute_tool.assert_called_with("search_course_content", query="test query")
    ...
    def test_sequential_tool_calling_tool_failure_stops_rounds(self, mock_tool_manager):
        ...
        with patch('ai_generator.anthropic.Anthropic') as mock_anthropic:
            ...
            assert "Error: Tool execution failed" in tool_result_message["content"][0]["content"]